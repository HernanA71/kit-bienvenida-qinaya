...
function clasificarSeveridad(estado, incidencia) {
    ...
    // Categoría: OK (Solo si dice explícitamente "en funcionamiento")
    if (texto.includes('en funcionamiento')) {
        return 'ok';
    }
    
    // Por defecto para cualquier otro estado (ej: "en espera", "pendiente")
    return 'ok';
}
...
document.addEventListener('DOMContentLoaded', async () => {
    try {
        ...
        if (data && Array.isArray(data) && data.length > 0) {
            ...
            // Filtro de Exclusión desactivado a petición del usuario para que se incluyan todos los colegios en las métricas
...
function updateKPIs() {
    ...
    if (colEl) colEl.textContent = `${totalColegiosCount} de ${totalRegistros}`;
